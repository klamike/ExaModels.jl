...
import ExaModels: ExaModels, NLPModels, ParametricNLPModels
...
function ExaModels.ExaModel(
    ...
    return ExaModels.ExaModel(
        ...
        ExaModels._parametric_meta(c; jpprod_available = prod, jptprod_available = prod, hpprod_available = prod, hptprod_available = prod),
    )
end
...
function _obj_mhess_structure!(backend, objs, rows, cols)
    ExaModels.smhessian!(backend, rows, cols, objs, nothing, nothing, NaN)
...
function _obj_mhess_structure!(backend, objs, sparsity, ::Nothing)
    ExaModels.smhessian!(backend, sparsity, nothing, objs, nothing, nothing, NaN)
...
function _con_mhess_structure!(backend, cons, rows, cols)
    ExaModels.smhessian!(backend, rows, cols, cons, nothing, nothing, NaN)
...
function _con_mhess_structure!(backend, cons, sparsity, ::Nothing)
    ExaModels.smhessian!(backend, sparsity, nothing, cons, nothing, nothing, NaN)
...
function ExaModels.smhessian!(backend::B, y1, y2, f, x, θ, adj) where {B<:KernelAbstractions.Backend}
    if !isempty(f.itr)
        kermh(backend)(y1, y2, f.f, f.itr, x, θ, adj; ndrange = length(f.itr))
    end
end
function ExaModels.smhessian!(backend::B, y1, y2, f, x, θ, adj::V) where {B<:KernelAbstractions.Backend,V<:AbstractVector}
    if !isempty(f.itr)
...
function ExaModels.sjacobianp!(backend::B, y1, y2, f, x, θ, adj) where {B<:KernelAbstractions.Backend}
    if !isempty(f.itr)
...
function _grad_param_ka!(backend, g, objs, x, θ)
    ...
    ExaModels.sgradientp!(backend, g, objs, x, θ, one(eltype(g)))
end
function _grad_param_ka!(backend, g, objs::ExaModels.ObjectiveNull, x, θ) end

function ExaModels.sgradientp!(backend::B, y, f, x, θ, adj) where {B<:KernelAbstractions.Backend}
    if !isempty(f.itr)
...
function _grad_param_structure!(backend, objs, pgsparsity)
    ExaModels.sgradientp!(backend, pgsparsity, objs, nothing, nothing, NaN)
...
@kernel function kerjp(y1, y2, @Const(f), @Const(itr), @Const(x), @Const(θ), @Const(adj))
    I = @index(Global)
    @inbounds ExaModels.jrpass(f(itr[I], x, ExaModels.AdjointParameterSource(θ)), f.pcomp1, ExaModels.offset0(f, itr, I), y1, y2, ExaModels.poffset1(f, I), 0, adj)
end
...
@kernel function kermh(y1, y2, @Const(f), @Const(itr), @Const(x), @Const(θ), @Const(adj))
    ...
    @inbounds ExaModels.mhrpass0(
        ...
        f.mcomp2, y1, y2, ExaModels.moffset2(f, I), 0, adj,
    )
end
@kernel function kermh2(y1, y2, @Const(f), @Const(itr), @Const(x), @Const(θ), @Const(adjs))
    ...
    @inbounds ExaModels.mhrpass0(
        f(itr[I], ExaModels.SecondAdjointNodeSource(x), ExaModels.SecondAdjointParameterSource(θ)),
        f.mcomp2, y1, y2, ExaModels.moffset2(f, I), 0, adjs[ExaModels.offset0(f, itr, I)],
    )
end
