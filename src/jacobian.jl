...
@inline function jrpass(
    ...
    cnt,
    adj,
) where {D<:AdjointNode,V1<:AbstractVector,V2<:AbstractVector,W1<:AbstractVector,W2<:AbstractVector}
    y_fwd, v_fwd = y1
    @inbounds y_fwd[i] += adj * v_fwd[d.i]
    y_t, v_t = y2
    @inbounds y_t[d.i] += adj * v_t[i]
    return (cnt += 1)
end
@inline function jrpass(
    d::D,
...
"""
    sjacobian_param!(y1, y2, f, x, θ, adj)
...
- `adj`: initial adjoint
"""
function sjacobian_param!(y1, y2, f, x, θ, adj)
    @simd for i in eachindex(f.itr)
        @inbounds sjacobian_param!(
            y1,
            y2,
            f.f,
            f.itr[i],
...
function sjacobian_param!(y1, y2, f, p, x, θ, comp, o0, po1, adj)
