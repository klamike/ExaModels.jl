...
- `adj`: initial adjoint
"""
function sgradient_param!(y, f, x, θ, adj)
    @simd for k in eachindex(f.itr)
        @inbounds sgradient_param!(y, f.f, f.itr[k], x, θ, f.f.pcomp1, offset1_param(f, k), adj)
    end
    return y
end

function sgradient_param!(y, f, p, x, θ, comp, po1, adj)
    graph = f(p, x, AdjointParameterSource(θ))
